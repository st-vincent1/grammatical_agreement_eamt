# todo: needs to be tested and pruned
...
from src.data import load_train_dev, add_tags_to_vocab, read_vocab, save_vocab, build_context_vocab
...
def main():
    ...
    parser.add_argument('--alpha', default=0.25, type=float, help='Regularisation parameter.')
    ...
    if params.config in ['emb_add', 'out_bias']:
        params.ft.lr *= 2
    ...
    optimizer = optim.Adam(model.parameters(), lr=params.ft.lr, weight_decay=params.ft.L2)
    model.config = params.config
    print(f"{model.decoder.generator.config=}")
    params, train_iter, dev_iter, vocab = load_train_dev(params, vocab, model.cxt_vocab)
    train(params, vocab, model, train_iter, dev_iter, optimizer, 0, cxt_vocab=model.cxt_vocab)
